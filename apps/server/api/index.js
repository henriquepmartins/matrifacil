...
// No Vercel, as variáveis de ambiente são fornecidas diretamente pelo sistema
if (process.env.NODE_ENV !== "production") {
  dotenv.default.config({
    path: path.default.resolve(process.cwd(), "apps/server/.env"),
  });
}
 ...
 */
router$5.get("/readiness", (req, res, next) =>
  healthController.readiness(req, res, next)
);
/**
 * GET /health/liveness - Liveness probe
 */
router$5.get("/liveness", (req, res, next) =>
...
var UserRepository = class {
  ...
  async createUser(data) {
    const [newUser] = await require_dist.db
      .insert(require_dist.user)
      .values({
        id: data.id,
        name: data.name,
        email: data.email,
  ...
  async findUserByEmail(email) {
    const [foundUser] = await require_dist.db
      .select()
      .from(require_dist.user)
      .where((0, drizzle_orm.eq)(require_dist.user.email, email))
      .limit(1);
    return foundUser;
  }
  /**
  ...
  async findUserById(id) {
    const [foundUser] = await require_dist.db
      .select()
      .from(require_dist.user)
      .where((0, drizzle_orm.eq)(require_dist.user.id, id))
      .limit(1);
    return foundUser;
  }
  /**
  ...
  async findUserPassword(userId) {
    const [userAccount] = await require_dist.db
      .select()
      .from(require_dist.account)
      .where(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.eq)(require_dist.account.userId, userId),
          (0, drizzle_orm.eq)(require_dist.account.providerId, "credential")
        )
      )
      .limit(1);
  ...
  async createSession(data) {
    const [newSession] = await require_dist.db
      .insert(require_dist.session)
      .values({
        id: data.id,
        userId: data.userId,
        token: data.token,
  ...
  async findSessionByToken(token) {
    const [foundSession] = await require_dist.db
      .select()
      .from(require_dist.session)
      .where((0, drizzle_orm.eq)(require_dist.session.token, token))
      .limit(1);
    return foundSession;
  }
  /**
  ...
  async deleteSession(sessionId) {
    await require_dist.db
      .delete(require_dist.session)
      .where((0, drizzle_orm.eq)(require_dist.session.id, sessionId));
  }
  /**
  ...
  async deleteExpiredSessions() {
    ...
    await require_dist.db
      .delete(require_dist.session)
      .where((0, drizzle_orm.eq)(require_dist.session.expiresAt, now));
  }
  /**
  ...
  async updateUserLastAccess(userId) {
    await require_dist.db
      .update(require_dist.user)
      .set({ updatedAt: /* @__PURE__ */ new Date() })
      .where((0, drizzle_orm.eq)(require_dist.user.id, userId));
  }
};
...
var AuthService = class {
  ...
  async signUp(data, sessionInfo) {
    if (await userRepository.findUserByEmail(data.email))
  ...
  async signIn(data, sessionInfo) {
    ...
    if (!(await bcrypt.default.compare(data.password, hashedPassword)))
...
function authenticateToken(req, res, next) {
  try {
    ...
    if (error instanceof jsonwebtoken.default.JsonWebTokenError)
      next(new AppError(401, "Token inválido"));
    else if (error instanceof jsonwebtoken.default.TokenExpiredError)
...
var require_db = /* @__PURE__ */ require_dist.__commonJS({
  "src/db/index.js": (exports, module) => {
    ...
    module.exports.initializeDatabase = async function () {
      ...
      if (!(await module.exports.checkDatabaseConnection()))
...
var DrizzleAlunoRepository = class {
  async findById(id) {
    const [result] = await import_db.db
      .select()
      .from(require_dist.aluno)
      .where((0, drizzle_orm.eq)(require_dist.aluno.id, id))
      .limit(1);
  ...
  async findByMatriculaId(matriculaId) {
    const [result] = await import_db.db
      ...
      .from(require_dist.aluno)
      .innerJoin(
        require_dist.matricula,
        (0, drizzle_orm.eq)(
          require_dist.aluno.id,
          require_dist.matricula.alunoId
        )
      )
      .where((0, drizzle_orm.eq)(require_dist.matricula.id, matriculaId))
      .limit(1);
    if (!result) return null;
    return Aluno.create({
      id: result.aluno.id,
  ...
  async update(alunoEntity) {
    const [result] = await import_db.db
      ...
      .where((0, drizzle_orm.eq)(require_dist.aluno.id, alunoEntity.id))
      .returning();
    return Aluno.create({
      id: result.id,
      idGlobal: result.idGlobal,
...
var Matricula = class Matricula {
  ...
  aprovar() {
    return new Matricula(
      ...
      this.turma,
      "completo",
      this.dataMatricula || /* @__PURE__ */ new Date(),
      this.observacoes,
      this.createdAt,
      /* @__PURE__ */ new Date()
    );
...
var DrizzleMatriculaRepository = class {
  async findById(id) {
    const [result] = await import_db.db
      ...
      .leftJoin(
        require_dist.responsavel,
        (0, drizzle_orm.eq)(
          require_dist.matricula.responsavelId,
          require_dist.responsavel.id
        )
      )
      .leftJoin(
        require_dist.turma,
        (0, drizzle_orm.eq)(
          require_dist.matricula.turmaId,
          require_dist.turma.id
        )
      )
  ...
  async findByStatus(status) {
    return (
      await import_db.db
        ...
        .where((0, drizzle_orm.eq)(require_dist.matricula.status, status))
    ).map(this.mapToMatricula);
  }
  async findByEtapa(etapa) {
    return (
      await import_db.db
        ...
        .where((0, drizzle_orm.eq)(require_dist.aluno.etapa, etapa))
    ).map(this.mapToMatricula);
  }
  async findBySearch(search) {
    return (
      await import_db.db
        ...
        .where(
          (0, drizzle_orm.or)(
            (0, drizzle_orm.like)(require_dist.aluno.nome, `%${search}%`),
            (0, drizzle_orm.like)(require_dist.responsavel.nome, `%${search}%`),
            (0, drizzle_orm.like)(
              require_dist.matricula.protocoloLocal,
              `%${search}%`
            )
          )
        )
  ...
  async findAll(filters) {
    ...
    if (filters?.status && filters.status !== "todos")
      query = query.where(
        (0, drizzle_orm.eq)(require_dist.matricula.status, filters.status)
      );
    if (filters?.etapa && filters.etapa !== "todos")
      query = query.where(
        (0, drizzle_orm.eq)(require_dist.aluno.etapa, filters.etapa)
      );
    if (filters?.search) {
      const searchTerm = `%${filters.search}%`;
      query = query.where(
        (0, drizzle_orm.or)(
          (0, drizzle_orm.like)(require_dist.aluno.nome, searchTerm),
          (0, drizzle_orm.like)(require_dist.responsavel.nome, searchTerm),
          (0, drizzle_orm.like)(
            require_dist.matricula.protocoloLocal,
            searchTerm
          )
    ...
    query = query.orderBy(
      drizzle_orm.sql`${require_dist.matricula.createdAt} DESC`
    );
  ...
  async count(filters) {
    let query = import_db.db
      .select({ count: drizzle_orm.sql`count(*)` })
      .from(require_dist.matricula)
      .leftJoin(
        require_dist.aluno,
        (0, drizzle_orm.eq)(
  ...
  async update(matriculaEntity) {
    await import_db.db
      ...
      .where(
        (0, drizzle_orm.eq)(require_dist.matricula.id, matriculaEntity.id)
      );
    await import_db.db
      .update(require_dist.aluno)
      .set({
      ...
      .where(
        (0, drizzle_orm.eq)(require_dist.aluno.id, matriculaEntity.aluno.id)
      );
    return this.findById(matriculaEntity.id);
  }
  async delete(id) {
    await import_db.db
      .delete(require_dist.matricula)
      .where((0, drizzle_orm.eq)(require_dist.matricula.id, id));
  }
  async createPreMatricula(data) {
    const protocolo = await this.generateProtocolo();
    const turmaDisponivel = await import_db.db
      .select()
      .from(require_dist.turma)
      .where(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.eq)(require_dist.turma.etapa, data.aluno.etapa),
          (0, drizzle_orm.eq)(require_dist.turma.ativa, true),
          drizzle_orm.sql`${require_dist.turma.vagasDisponiveis} > 0`
        )
      )
      .limit(1);
    const turmaId = turmaDisponivel.length > 0 ? turmaDisponivel[0].id : null;
    const alunoId = v4_default();
    const [newAluno] = await import_db.db
      .insert(require_dist.aluno)
      .values({
        id: alunoId,
        idGlobal: v4_default(),
    ...
    const [newResponsavel] = await import_db.db
      .insert(require_dist.responsavel)
      .values({
        id: responsavelId,
        idGlobal: v4_default(),
    ...
    const [newMatricula] = await import_db.db
      .insert(require_dist.matricula)
      .values({
        id: matriculaId,
        idGlobal: v4_default(),
  ...
  async generateProtocolo() {
    ...
    const existingProtocols = await import_db.db
      .select({ protocoloLocal: require_dist.matricula.protocoloLocal })
      .from(require_dist.matricula)
      .where(
        (0, drizzle_orm.like)(
          require_dist.matricula.protocoloLocal,
      ...
      .orderBy(drizzle_orm.sql`${require_dist.matricula.protocoloLocal} DESC`);
    let nextNumber = 1;
    if (existingProtocols.length > 0) {
      const match =
        existingProtocols[0].protocoloLocal.match(/PRE-\d{4}-(\d{3})/);
...
var DrizzleResponsavelRepository = class {
  async findById(id) {
    const [result] = await import_db.db
      .select()
      .from(require_dist.responsavel)
      .where((0, drizzle_orm.eq)(require_dist.responsavel.id, id))
      .limit(1);
    if (!result) return null;
  ...
  async findByCPF(cpf) {
    const [result] = await import_db.db
      .select()
      .from(require_dist.responsavel)
      .where((0, drizzle_orm.eq)(require_dist.responsavel.cpf, cpf))
      .limit(1);
    if (!result) return null;
    return Responsavel.create({
      id: result.id,
  ...
  async update(responsavelEntity) {
    const [result] = await import_db.db
      .update(require_dist.responsavel)
      .set({
        nome: responsavelEntity.nome,
        cpf: responsavelEntity.cpf,
        telefone: responsavelEntity.telefone,
      ...
      .where(
        (0, drizzle_orm.eq)(require_dist.responsavel.id, responsavelEntity.id)
      )
      .returning();
    return Responsavel.create({
      id: result.id,
  ...
  async delete(id) {
    await import_db.db
      .delete(require_dist.responsavel)
      .where((0, drizzle_orm.eq)(require_dist.responsavel.id, id));
  }
};
...
var DrizzleRelatorioRepository = class {
  ...
  async findRecentReports(usuarioId, limit = 10, offset = 0) {
    return (
      await require_dist.db
        .select()
        .from(require_dist.relatorioGerado)
        .where(
          (0, drizzle_orm.eq)(require_dist.relatorioGerado.usuarioId, usuarioId)
        )
        .orderBy((0, drizzle_orm.desc)(require_dist.relatorioGerado.createdAt))
        .limit(limit)
        .offset(offset)
    ).map((report) => ({
      id: report.id,
  ...
  async getMatriculasData(filtros) {
    const {
      ...
      turma: turma$1,
    } = await Promise.resolve().then(() => require("./dist-BiQyyt1H.js"));
    let query = require_dist.db
      .select({
        id: matricula$1.id,
      ...
      .from(matricula$1)
      .innerJoin(aluno$1, (0, drizzle_orm.eq)(matricula$1.alunoId, aluno$1.id))
      .innerJoin(
        responsavel$1,
        (0, drizzle_orm.eq)(matricula$1.responsavelId, responsavel$1.id)
      )
      .leftJoin(turma$1, (0, drizzle_orm.eq)(matricula$1.turmaId, turma$1.id));
    const conditions = [];
    if (filtros.dataInicio && filtros.dataFim) {
      const campoData =
        filtros.campoData === "dataMatricula"
          ? matricula$1.dataMatricula
          : matricula$1.createdAt;
      conditions.push(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.gte)(campoData, filtros.dataInicio),
          (0, drizzle_orm.lte)(campoData, filtros.dataFim)
        )
      );
    }
    if (filtros.status && filtros.status !== "todos")
      conditions.push((0, drizzle_orm.eq)(matricula$1.status, filtros.status));
    if (filtros.etapa && filtros.etapa !== "todos")
      conditions.push((0, drizzle_orm.eq)(aluno$1.etapa, filtros.etapa));
    if (filtros.turma && filtros.turma !== "todos")
      conditions.push((0, drizzle_orm.eq)(turma$1.id, filtros.turma));
    if (filtros.search)
      conditions.push(
        (0, drizzle_orm.or)(
          (0, drizzle_orm.ilike)(aluno$1.nome, `%${filtros.search}%`),
          (0, drizzle_orm.ilike)(responsavel$1.nome, `%${filtros.search}%`),
          (0, drizzle_orm.ilike)(
            matricula$1.protocoloLocal,
            `%${filtros.search}%`
          )
        )
      );
    if (conditions.length > 0)
      query = query.where((0, drizzle_orm.and)(...conditions));
    return await query;
  }
  ...
  async getTurmasData(filtros) {
    const { turma: turma$1 } = await Promise.resolve().then(() =>
      require("./dist-BiQyyt1H.js")
    );
    const {
      eq: eq$7,
      and: and$5,
    ...
    const conditions = [];
    if (filtros.etapa && filtros.etapa !== "todos")
      conditions.push(eq$7(turma$1.etapa, filtros.etapa));
    if (filtros.turma && filtros.turma !== "todos")
      conditions.push(eq$7(turma$1.id, filtros.turma));
    if (filtros.search)
      conditions.push(ilike$1(turma$1.nome, `%${filtros.search}%`));
    if (conditions.length > 0) query = query.where(and$5(...conditions));
    const turmas = await query;
    const { matricula: matricula$1 } = await Promise.resolve().then(() =>
      require("./dist-BiQyyt1H.js")
    );
    const turmasComContagem = [];
    for (const turma$2 of turmas) {
      const alunosCount = await require_dist.db
        .select({ count: (0, drizzle_orm.count)() })
        .from(matricula$1)
        .where(eq$7(matricula$1.turmaId, turma$2.id));
  ...
  async getDocumentosData(filtros) {
    ...
    let query = require_dist.db
      ...
      .innerJoin(
        matricula$1,
        (0, drizzle_orm.eq)(documento$1.matriculaId, matricula$1.id)
      )
      .innerJoin(aluno$1, (0, drizzle_orm.eq)(matricula$1.alunoId, aluno$1.id))
      .innerJoin(
        responsavel$1,
    ...
    if (filtros.dataInicio && filtros.dataFim)
      conditions.push(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.gte)(documento$1.createdAt, filtros.dataInicio),
          (0, drizzle_orm.lte)(documento$1.createdAt, filtros.dataFim)
        )
      );
    if (filtros.status && filtros.status !== "todos")
      conditions.push((0, drizzle_orm.eq)(documento$1.status, filtros.status));
    if (filtros.search)
      conditions.push(
        (0, drizzle_orm.or)(
          (0, drizzle_orm.ilike)(aluno$1.nome, `%${filtros.search}%`),
  ...
  async getPendenciasData(filtros) {
    ...
    let query = require_dist.db
      ...
      .innerJoin(
        matricula$1,
        (0, drizzle_orm.eq)(pendencia$1.matriculaId, matricula$1.id)
      )
      .innerJoin(aluno$1, (0, drizzle_orm.eq)(matricula$1.alunoId, aluno$1.id))
      .innerJoin(
        responsavel$1,
    ...
    if (filtros.dataInicio && filtros.dataFim)
      conditions.push(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.gte)(pendencia$1.createdAt, filtros.dataInicio),
          (0, drizzle_orm.lte)(pendencia$1.createdAt, filtros.dataFim)
        )
      );
    if (filtros.status && filtros.status !== "todos")
      conditions.push(
        (0, drizzle_orm.eq)(
          pendencia$1.resolvido,
          filtros.status === "resolvido"
        )
      );
    if (filtros.search)
      conditions.push(
        (0, drizzle_orm.or)(
          ...
          (0, drizzle_orm.ilike)(pendencia$1.descricao, `%${filtros.search}%`)
        )
      );
    if (conditions.length > 0)
      query = query.where((0, drizzle_orm.and)(...conditions));
...
var MatriculaDomainService = class {
  ...
  assignTurmaToMatricula(matricula$1, turma$1) {
    if (!turma$1.temVagasDisponiveis())
      throw new Error("Turma não possui vagas disponíveis");
    if (turma$1.etapa !== matricula$1.aluno.etapa)
      throw new Error("Turma não é compatível com a etapa do aluno");
    return new Matricula(
      matricula$1.id,
      matricula$1.idGlobal,
      matricula$1.protocoloLocal,
      matricula$1.aluno,
      matricula$1.responsavel,
      turma$1,
      matricula$1.status,
      matricula$1.dataMatricula,
      matricula$1.observacoes,
      matricula$1.createdAt,
      /* @__PURE__ */ new Date()
    );
  }
...
var PreMatriculaRepository = class {
  ...
  async updateMatricula(id, data) {
    ...
    if (
      ...
      data.alunoEtapa ||
      data.alunoNecessidadesEspeciais !== void 0 ||
      data.alunoObservacoes !== void 0
    )
      await require_dist.db
        .update(require_dist.aluno)
        .set({
    ...
    if (
      ...
      await require_dist.db
        ...
        .where(
          (0, drizzle_orm.eq)(
            require_dist.responsavel.id,
            existing.responsavel.id
          )
        );
    if (data.observacoes !== void 0)
      await require_dist.db
        .update(require_dist.matricula)
  ...
  async deleteMatricula(id) {
    ...
    try {
      await require_dist.db
        .delete(require_dist.documento)
        .where((0, drizzle_orm.eq)(require_dist.documento.matriculaId, id));
      await require_dist.db
        .delete(require_dist.matricula)
        .where((0, drizzle_orm.eq)(require_dist.matricula.id, id));
      return true;
  ...
  async findResponsavelByCPF(cpfValue) {
    const [result] = await require_dist.db
      .select({ id: require_dist.responsavel.id })
      .from(require_dist.responsavel)
      .where((0, drizzle_orm.eq)(require_dist.responsavel.cpf, cpfValue))
      .limit(1);
    return result || null;
  }
  async generateProtocolo() {
    const year = /* @__PURE__ */ new Date().getFullYear();
    const existingProtocols = await require_dist.db
      .select({ protocoloLocal: require_dist.matricula.protocoloLocal })
      .from(require_dist.matricula)
      .where(
        (0, drizzle_orm.like)(
      ...
      .orderBy((0, drizzle_orm.desc)(require_dist.matricula.protocoloLocal));
    let nextNumber = 1;
    if (existingProtocols.length > 0) {
      const match =
        existingProtocols[0].protocoloLocal.match(/PRE-\d{4}-(\d{3})/);
  ...
  async createPreMatricula(data) {
    const protocolo = await this.generateProtocolo();
    const alunoId = v4_default();
    const [newAluno] = await require_dist.db
      .insert(require_dist.aluno)
      .values({
        id: alunoId,
        idGlobal: v4_default(),
    ...
    const matriculaId = v4_default();
    const [newMatricula] = await require_dist.db
      .insert(require_dist.matricula)
      .values({
        id: matriculaId,
        idGlobal: v4_default(),
  ...
  async findAll(filters = {}) {
    let query = require_dist.db
      ...
      .where((0, drizzle_orm.eq)(require_dist.matricula.status, "pre"))
      .orderBy((0, drizzle_orm.desc)(require_dist.matricula.createdAt));
    if (filters.etapa)
      query = query.where(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.eq)(require_dist.matricula.status, "pre"),
          (0, drizzle_orm.eq)(require_dist.aluno.etapa, filters.etapa)
        )
      );
    if (filters.search) {
  ...
  async findById(id) {
    const [result] = await require_dist.db
      ...
      .where(
        (0, drizzle_orm.and)(
          (0, drizzle_orm.eq)(require_dist.matricula.id, id),
          (0, drizzle_orm.eq)(require_dist.matricula.status, "pre")
        )
      )
      .limit(1);
  ...
  async convertToMatriculaCompleta(
    ...
    if (!finalTurmaId)
      finalTurmaId =
        (await this.findBestTurmaForEtapa(existing.aluno.etapa))?.id || null;
    await require_dist.db
      .update(require_dist.matricula)
      .set({
        status: "completo",
    ...
    if (documentosIniciais && documentosIniciais.length > 0)
      await require_dist.db.insert(require_dist.documento).values(
  ...
  async count(filters = {}) {
    let query = require_dist.db
      .select({ count: require_dist.matricula.id })
      .from(require_dist.matricula)
      .leftJoin(
        require_dist.aluno,
        (0, drizzle_orm.eq)(
...
var CreatePreMatriculaUseCase = class {
  ...
  async execute(request) {
    ...
    return {
      matricula: await preMatriculaRepository.createPreMatricula(request),
    };
  }
};
...
var ConvertToMatriculaCompletaUseCase = class {
  ...
  async execute(request) {
    const matricula$1 = await this.matriculaRepository.findById(
      request.matriculaId
    );
    if (!matricula$1) throw new Error("Pré-matrícula não encontrada");
    if (!this.domainService.canConvertToMatriculaCompleta(matricula$1))
    ...
    if (request.turmaId) {
      ...
      turma$1 = await this.turmaRepository.findBestForEtapa(
        matricula$1.aluno.etapa
      );
      if (!turma$1) throw new Error("Nenhuma turma disponível para esta etapa");
    }
    const matriculaCompleta = matricula$1.converterParaCompleta(
      turma$1,
      request.dataMatricula
    );
    return {
      matricula: await this.matriculaRepository.update(matriculaCompleta),
    };
  }
};
...
var ApproveMatriculaUseCase = class {
  ...
  async execute(request) {
    ...
    if (!this.domainService.canApproveMatricula(matricula$1))
      throw new Error("Matrícula já está aprovada");
    const matriculaAprovada = matricula$1.aprovar();
    return {
      matricula: await this.matriculaRepository.update(matriculaAprovada),
    };
  }
};
...
var GetDashboardStatsUseCase = class {
  ...
  async execute() {
    const [
      ...
      import_db.db
        .select({ count: drizzle_orm.sql`count(*)` })
        .from(require_dist.documento)
        .where((0, drizzle_orm.eq)(require_dist.documento.status, "pendente")),
      import_db.db
        .select({
        ...
        .from(require_dist.turma)
        .where((0, drizzle_orm.eq)(require_dist.turma.ativa, true)),
      import_db.db
        .select({ count: drizzle_orm.sql`count(*)` })
        .from(require_dist.matricula)
        .where((0, drizzle_orm.eq)(require_dist.matricula.status, "completo")),
      import_db.db
        .select({ count: drizzle_orm.sql`count(*)` })
        .from(require_dist.matricula)
        .where(
          (0, drizzle_orm.eq)(require_dist.matricula.status, "pendente_doc")
        ),
      import_db.db
        .select({ count: drizzle_orm.sql`count(*)` })
        .from(require_dist.aluno),
...
var PreMatriculaService = class {
  ...
  async updatePreMatricula(id, data) {
    ...
    if (
      ...
      if (
        (await preMatriculaRepository.findAll({ search: data.responsavel.cpf }))
          .length > 0
      )
  ...
  async deletePreMatricula(id) {
    if (!id) throw new AppError(400, "ID da pré-matrícula é obrigatório");
    if (!(await preMatriculaRepository.deletePreMatricula(id)))
  ...
  async deleteMatricula(id) {
    if (!id) throw new AppError(400, "ID da matrícula é obrigatório");
    if (!(await preMatriculaRepository.deleteMatricula(id)))
...
router$1.post("/apply-migration", async (req, res) => {
  try {
    ...
    await require_dist.db.execute(
      drizzle_orm.sql`ALTER TABLE "aluno" ALTER COLUMN "nacionalidade" DROP DEFAULT`
    );
    await require_dist.db.execute(
      drizzle_orm.sql`ALTER TABLE "responsavel" ALTER COLUMN "nacionalidade" DROP DEFAULT`
    );
...
router$1.post("/create-turmas", async (req, res) => {
  try {
    ...
    for (const turmaData of turmas)
      await require_dist.db.insert(require_dist.turma).values(turmaData);
  ...
  try {
    ...
    for (const matricula$1 of matriculasSemTurma.rows) {
      ...
      if (turmasDisponiveis.length > 0) {
        ...
        atualizadas++;
        console.log(
          `✅ Matrícula ${matricula$1.id} associada à turma ${turmaEscolhida.nome}`
        );
      }
    }
...
router$1.post("/test-relatorio-gerar", async (req, res) => {
  try {
    ...
    if (formato === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="relatorio_matriculas_${/* @__PURE__ */ new Date()
          .toISOString()
          .slice(0, 10)}.csv"`
      );
      res.send(buffer);
...
router$1.post("/test-relatorio-turmas-gerar", async (req, res) => {
  try {
    ...
    if (formato === "csv") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="relatorio_turmas_${/* @__PURE__ */ new Date()
          .toISOString()
          .slice(0, 10)}.csv"`
      );
      res.send(buffer);
