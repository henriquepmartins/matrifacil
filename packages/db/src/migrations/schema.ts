...
export const statusDocumento = pgEnum("status_documento", ['pendente', 'anexado', 'aprovado'])
export const statusMatricula = pgEnum("status_matricula", ['pre', 'pendente_doc', 'completo', 'concluido'])
export const tipoDocumento = pgEnum("tipo_documento", ['certidao', 'rg_cpf_resp', 'vacina', 'residencia', 'historico', 'foto3x4'])
export const tipoRelatorio = pgEnum("tipo_relatorio", ['matriculas', 'pre_matriculas', 'turmas', 'documentos', 'pendencias', 'geral'])
export const turno = pgEnum("turno", ['manha', 'tarde', 'integral'])
...
export const aluno = pgTable("aluno", {
	...
	unique("aluno_id_global_unique").on(table.idGlobal),
]);
...
export const turma = pgTable("turma", {
	...
	unique("turma_id_global_unique").on(table.idGlobal),
]);
...
export const responsavel = pgTable("responsavel", {
	...
	unique("responsavel_id_global_unique").on(table.idGlobal),
	unique("responsavel_cpf_unique").on(table.cpf),
]);
